...
int main() {
    ...
    Dcf_Pack(&prg, &key_host, uint128_t(0, random_num), 64, &k0, &k1, maxlayer);
    ...
    uint64_t* alpha2 = (uint64_t*)malloc(N * sizeof(uint64_t));
    ...
    bool* res1 = (bool*)malloc(N * sizeof(bool));
    bool* res2 = (bool*)malloc(N * sizeof(bool));
    ...
    for(uint64_t i = 0; i < N; i++){
        ...
        if( res1[i]^res2[i] ^ (i<=0x100)){
            printf("Error at i = %ld, res1[i]^res2[i] = %d, i<=0x100 = %d\n", i, res1[i]^res2[i], i<=0x100);
        }
    }
    printf("final_res = %d\n", final_res);
    end = std::chrono::high_resolution_clock::now();
